...
import { setLoading, setNextActionAllowance, update } from "./store/slices/restaurant.slice";
...
const App = () => {
  ...
  const isNextActionAllowed = useAppSelector((state) => state.restaurant.isNextActionAllowed);
  ...
  useEffect(() => {
    if (isNextActionAllowed) {
      async function initNextAction() {
        ...
        await sleep(2000);
        const { restaurants, characters, timeToNextAction } = await restaurantManager.manageRestaurants();
        let sleepTimer = timeToNextAction;
        if (sleepTimer < 0) {
          sleepTimer = 20000;
        }
        const payload = {
