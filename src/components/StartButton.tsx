...
import { setInitialize, setNextActionAllowance } from "../store/slices/restaurant.slice";

const StartButton = () => {
  const isInitialized = useAppSelector((state) => state.restaurant.isInitialized);
  const isStartButtonDisabled = useAppSelector((state) => state.restaurant.isStartButtonDisabled);
